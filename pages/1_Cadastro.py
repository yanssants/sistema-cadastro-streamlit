# page_1.py
# (Código corrigido com a lógica de on_click)
...
def app():
    ...
    with st.expander("Dados Pessoais", expanded=True):
        st.text_input("Nome *", placeholder="Ex: João da Silva", key="nome")
        st.radio("Tipo de Pessoa *", ["Com vínculo", "Candidato", "Liderança"], key="tipo_pessoa")
        
        if st.session_state.tipo_pessoa == "Com vínculo":
            st.text_input("Qual Vínculo? *", placeholder="Ex: Indicação do Vereador José", key="vinculo_descricao")
        
        if st.session_state.tipo_pessoa == "Liderança":
            st.text_input("Liderança de qual Candidato? *", placeholder="Ex: Maria Oliveira", key="candidato_lideranca")
    ...
    with st.expander("Localização", expanded=True):
        ...
        st.selectbox("Município *", municipios_para, key="municipio")
    ...
    with st.expander("Contato", expanded=True):
        st.text_input("Telefone *", placeholder="Ex: (91) 98765-4321", key="telefone")
    ...
    with st.expander("Assistência Solicitada", expanded=True):
        st.selectbox("Serviço Requerido *", [
        ...
        if st.session_state.tipo_ajuda == "Outros":
            st.text_area("Descreva o tipo de ajuda:", placeholder="Detalhe o serviço necessário", key="descricao_outros")
        
        st.text_area("Detalhes adicionais:", placeholder="Informe qualquer outra informação relevante", key="detalhes")
        st.number_input("Quantidade *", min_value=1, step=1, key="quantidade")
        st.number_input("Valor (R$) *", min_value=0.0, format="%.2f", key="valor")
        st.caption("Preencha as informações sobre a assistência solicitada.")
    
    st.markdown("---")

    # O botão agora apenas chama a função de processamento ao ser clicado.
    st.button("Enviar", type="primary", on_click=processar_envio)
